...
char usage_str[] = "Usage: smx_perf_pub [-h] [-a affinity_cpu] [-c config] [-f flags] [-j jitter_loops] [-l linger_ms] [-m msg_len] [-n num_msgs] [-r rate] [-T topic]";
...
void help() {
  ...
  fprintf(stderr, "where:\n"
      "  -h : print help\n"
      "  -a affinity_cpu : bitmap for CPU affinity for send thread [0x%04x]\n"
      "  -c config : configuration file; can be repeated [%s]\n"
      "  -f flags : bitmap: 0x01: FLAGS_TIMESTAMP (for latency measures)\n"
      "                     0x02: FLAGS_NON_BLOCKING\n"
      "  -j jitter_loops : jitter measurement loops [%s]\n"
      ...
      o_flags, o_jitter_loops, o_msg_len, o_num_msgs, o_rate, o_topic
  );
  exit(0);
}
...
void get_my_opts(int argc, char **argv)
{
  ...
  while ((opt = getopt(argc, argv, "ha:c:f:j:l:m:n:r:t:")) != EOF) {
    switch (opt) {
      case 'h': help(); break;
...
int send_loop(lbm_src_t *src, int num_sends, uint64_t sends_per_sec, int flags)
{
  ...
  /* Get buffer from shared memory to construct message. */
...
int main(int argc, char **argv)
{
  ...
  printf("o_affinity_cpu=%d, o_config=%s, o_flags=0x%02x, o_jitter_loops=%d, o_linger_ms=%d, o_msg_len=%d, o_num_msgs=%d, o_rate=%d, o_topic='%s', \n",
      o_affinity_cpu, o_config, o_flags, o_jitter_loops, o_linger_ms, o_msg_len, o_num_msgs, o_rate, o_topic);
